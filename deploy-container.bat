...
REM echo Image Name: %IMAGE_NAME%
echo Application Port: %CONTAINER_PORT%
echo ========================================

REM Conectando ao servidor via SSH e executando os comandos
ssh root@%SERVER_IP% "cd docker && docker load -i %CONTAINER_NAME%.tar && docker run -d -p %CONTAINER_PORT%:3000 --name %CONTAINER_NAME% %CONTAINER_NAME%"
